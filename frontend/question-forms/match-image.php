<div class="space-y-6">
  <header class="space-y-2">
    <h2 class="text-2xl font-semibold text-gray-800">Tambah Soalan Padankan Perkataan & Gambar</h2>
  ...
  <section class="grid gap-6 lg:grid-cols-[2fr,3fr]">
    <div class="space-y-4">
      <div>
        ...
        <input id="matchInstruction" name="match_instruction" type="text"
               placeholder="Contoh: Padankan perkataan dengan gambar yang betul."
    ...
    <div class="space-y-4">
      <div class="flex items-center justify-between">
        <h3 class="text-sm font-medium text-gray-700">Senarai Padanan</h3>
  ...
  <section class="space-y-3">
    <div class="flex items-center justify-between">
      <h3 class="text-sm font-medium text-gray-700">Pratonton Padanan</h3>
      <button id="matchRefreshPreview" type="button"
    ...
    <div class="grid gap-4 md:grid-cols-2 bg-gray-50 border border-dashed border-gray-300 rounded-xl p-4">
      <div>
        ...
        <div id="matchWordPreview" class="space-y-2"></div>
      </div>
      <div>
        <p class="text-xs font-semibold text-gray-500 mb-2">Kolum Gambar</p>
        <div id="matchImagePreview" class="space-y-2"></div>
      </div>
    </div>
  </section>
...
<script>
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const wordPreview = document.getElementById("matchWordPreview");
    const imagePreview = document.getElementById("matchImagePreview");
    const refreshPreview = document.getElementById("matchRefreshPreview");

    if (!pairList || !addPair || !wordPreview || !imagePreview || !refreshPreview) {
      return;
    }
    ...
    const createPairRow = (index) => {
      ...
      imagePreviewBox.className = "match-image-preview h-24 w-full rounded-lg border border-dashed border-gray-300 bg-gray-50 flex items-center justify-center text-xs text-gray-400";
      imagePreviewBox.textContent = "Pratonton Gambar";
      imagePreviewBox.dataset.preview = "matchImage";

      const imageInput = document.createElement("input");
      imageInput.type = "file";
      imageInput.accept = "image/*";
      imageInput.name = "match_images[]";
    ...
    const renderPreview = () => {
      ...
      rows.forEach((row, index) => {
        ...
        const imageBox = row.querySelector("[data-preview='matchImage']");
